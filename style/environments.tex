...
%%
%% Counter discipline (standard textbook convention):
%%   - Definitions count independently:           Definition 1.1, 1.2, 1.3, ...
%%   - Examples count independently:               Example 1.1, 1.2, 1.3, ...
%%   - Theorems / Propositions / Lemmas / Corollaries share ONE counter
%%     (they are all "formal claims" of the same kind):
%%                                                 Theorem 1.1, Proposition 1.2,
%%                                                 Lemma 1.3, Corollary 1.4, ...
%% All counters reset per chapter.

% Three independent counters
\newcounter{mathemagicsDefCnt}[chapter]
\renewcommand{\themathemagicsDefCnt}{\thechapter.\arabic{mathemagicsDefCnt}}

\newcounter{mathemagicsThmCnt}[chapter]
\renewcommand{\themathemagicsThmCnt}{\thechapter.\arabic{mathemagicsThmCnt}}

\newcounter{mathemagicsExCnt}[chapter]
\renewcommand{\themathemagicsExCnt}{\thechapter.\arabic{mathemagicsExCnt}}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% (1) Definition, light blue tinted box, blue rule
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\newtcbtheorem[use counter=mathemagicsDefCnt]
...
\newtcbtheorem[use counter=mathemagicsThmCnt]
  {theorem}{Theorem}{%
    enhanced,breakable,
...
\newtcbtheorem[use counter=mathemagicsExCnt]
  {example}{Example}{%
    enhanced,breakable,
...
% Click-to-reveal answer (flashcard-style).
%
% Each \begin{warmupSolution}...\end{warmupSolution} produces a clickable
% "Reveal answer" button. The answer text is hidden by default in OCG-aware
% PDF viewers (Adobe Acrobat, pdf.js / VS Code LaTeX Workshop viewer, ...)
% and shown when the button is clicked. In viewers without OCG support, the
% layer is rendered "always" per the package option, so readers still see
% the answer.
%
% A separate counter (warmupocg) gives each warm-up a unique OCG layer ID.

\newcounter{warmupocg}

\newenvironment{warmupSolution}{%
  \stepcounter{warmupocg}%
  \edef\@currentwsoltag{wsol\thewarmupocg}%
  \par\smallskip\noindent%
  % The clickable toggle button
  \begingroup
    \sffamily\footnotesize
    \switchocg{\@currentwsoltag}{%
      \textcolor{mathemagicsLink}{\textbf{$\blacktriangleright$\ Reveal answer}}%
    }%
  \endgroup\par\smallskip
  % The hidden answer (initially OFF; printocg=always from class option)
  \begin{ocg}{Answer~\thewarmupocg}{\@currentwsoltag}{0}%
    \noindent{\sffamily\bfseries\small\itshape Answer:}\quad
    \small\ignorespaces%
}{%
    \par
  \end{ocg}%
